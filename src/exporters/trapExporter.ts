...
const applyEntriesHtml = (
    ...
    try {
        ...
        console.log(`[TrapExporter] ${dataType}:${id}:${locale} 生成 html 失败`);
    }
};
...
const buildEntity = (
    ...
    applyEntriesHtml(enOut, 'trap', id, 'en');
    applyEntriesHtml(zhOut, 'trap', id, 'zh');
...
export const runTrapExporter = async (): Promise<TrapExporterResult> => {
    ...
    const enRawEntries = Array.isArray(bilingual.en?.trap) ? [...bilingual.en.trap] : [];
    const zhRawEntries = Array.isArray(bilingual.zh?.trap) ? [...bilingual.zh.trap] : [];
    ...
    const fluffStore = buildFluffStore(
        Array.isArray(fluffBilingual.zh?.trapFluff) ? fluffBilingual.zh.trapFluff : [],
        Array.isArray(fluffBilingual.en?.trapFluff) ? fluffBilingual.en.trapFluff : []
    );
    ...
    const outputDir = path.join('./output', 'trap');
    ...
    for (const item of outputData) {
        ...
        const preferredFileName = `trap_1_${sourceId}_1_${baseName}.json`;
    ...
    const outputPath = path.join(namelistDir, 'trapnamelist.json');
    await fs.writeFile(outputPath, JSON.stringify(output, null, 2), 'utf-8');
    console.log(`已生成 trapnamelist.json 文件：${outputPath}`);
