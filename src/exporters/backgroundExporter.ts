...
const applyEntriesHtml = (
    ...
    try {
        ...
        console.log(`[BackgroundExporter] ${dataType}:${id}:${locale} 生成 html 失败`);
    }
};
...
const buildEntity = (
    ...
    applyEntriesHtml(enOut, 'background', id, 'en');
    applyEntriesHtml(zhOut, 'background', id, 'zh');
...
export const runBackgroundExporter = async (): Promise<BackgroundExporterResult> => {
    ...
    const bilingual = await loadBilingualFileCached('backgrounds.json', fileCache);
    const enRawEntries = Array.isArray(bilingual.en?.background) ? [...bilingual.en.background] : [];
    const zhRawEntries = Array.isArray(bilingual.zh?.background) ? [...bilingual.zh.background] : [];
    ...
    const fluffBilingual = await loadBilingualFileCached('fluff-backgrounds.json', fileCache);
    const fluffStore = buildFluffStore(
        Array.isArray(fluffBilingual.zh?.backgroundFluff) ? fluffBilingual.zh.backgroundFluff : [],
        Array.isArray(fluffBilingual.en?.backgroundFluff) ? fluffBilingual.en.backgroundFluff : []
    );
    ...
    const outputDir = path.join('./output', 'background');
    ...
    for (const item of outputData) {
        ...
        const preferredFileName = `background_1_${sourceId}_1_${baseName}.json`;
    ...
    const outputPath = path.join(namelistDir, 'backgroundnamelist.json');
    await fs.writeFile(outputPath, JSON.stringify(output, null, 2), 'utf-8');
    console.log(`已生成 backgroundnamelist.json 文件：${outputPath}`);
