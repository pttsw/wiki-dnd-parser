...
const applyEntriesHtml = (
    ...
    try {
        ...
        console.log(`[RaceExporter] ${dataType}:${id}:${locale} 生成 html 失败`);
    }
};
...
const buildEntity = (
    ...
    applyEntriesHtml(enOut, 'race', id, 'en');
    applyEntriesHtml(zhOut, 'race', id, 'zh');
...
export const runRaceExporter = async (): Promise<RaceExporterResult> => {
    ...
    const bilingual = await loadBilingualFileCached('races.json', fileCache);
    const enRawEntries = Array.isArray(bilingual.en?.race) ? [...bilingual.en.race] : [];
    const zhRawEntries = Array.isArray(bilingual.zh?.race) ? [...bilingual.zh.race] : [];
    ...
    const fluffBilingual = await loadBilingualFileCached('fluff-races.json', fileCache);
    const fluffStore = buildFluffStore(
        Array.isArray(fluffBilingual.zh?.raceFluff) ? fluffBilingual.zh.raceFluff : [],
        Array.isArray(fluffBilingual.en?.raceFluff) ? fluffBilingual.en.raceFluff : []
    );
    ...
    const outputDir = path.join('./output', 'race');
    ...
    for (const item of outputData) {
        ...
        const preferredFileName = `race_1_${sourceId}_1_${baseName}.json`;
    ...
    const outputPath = path.join(namelistDir, 'racenamelist.json');
    await fs.writeFile(outputPath, JSON.stringify(output, null, 2), 'utf-8');
    console.log(`已生成 racenamelist.json 文件：${outputPath}`);
