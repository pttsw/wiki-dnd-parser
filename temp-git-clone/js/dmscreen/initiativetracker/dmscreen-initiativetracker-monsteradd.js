...
class _InitiativeTrackerMonsterAddCustomizer extends BaseComponent {
	...
	_render_getEleFooter ({rdState}) {
		const btnSave = ee`<button class="ve-btn ve-btn-primary ve-btn-sm ve-w-100">保存</button>`
			.onn("click", () => {
				rdState.cbDoClose(
					true,
...
export class InitiativeTrackerMonsterAdd extends BaseComponent {
	...
	async pGetShowModalResults () {
		...
		const iptSearch = ee`<input class="ve-ui-search__ipt-search search ve-form-control" autocomplete="off" placeholder="搜索...">`;
		...
		ee`<div class="ve-split ve-no-shrink">
			...
			<label class="ve-ui-search__ipt-search-sub-wrp ve-flex-vh-center">${ComponentUiUtil.getCbBool(this, "isRollHp").addClass("ve-mr-1")} <span>随机 HP</span></label>
			...
			if (toProcess.length) {
				...
				if (resultCount > this.constructor._RESULTS_MAX_DISPLAY) {
					...
					if (I18nUtil.LANGUAGES_INDEX === "zh_CN") {
						wrpResults.appends(`<div class="ve-ui-search__row ve-ui-search__row--readonly">...${diff}条结果被隐藏，请调整搜索条件。</div>`);
					} else {
